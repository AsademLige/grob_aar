...
class IosARView: NSObject, FlutterPlatformView, ARSCNViewDelegate, UIGestureRecognizerDelegate, ARSessionDelegate {
    ...
    init(
        ...
        self.sceneView = SamMitiARView(frame: frame)
    ...
    func onObjectMethodCalled(_ call :FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        switch call.method {
            ...
            case "addNodeToPlaneAnchor":
                ...
                break
            case "toggleNode":
                if let name = arguments!["name"] as? String, let isEnabled = arguments!["value"] as? Bool {
                ...
                break
            case "togglePlayer":
                print("Not Avalible on IOS")
                result(nil)
                break
            case "setLight":
                if let name = arguments!["name"] as? String,
                    let light = arguments!["light"] as? Bool,
                    let intensity = arguments!["intensity"] as? Int,
                    ...
                    if (!light) {
                        ...
                        switch (type) {
                            case 0:
                                node!.light?.type = SCNLight.LightType.ambient
                                break
                            case 1:
                            ...
                            case 2:
                                ...
                                break
                            case 3:
                                node!.light?.type = SCNLight.LightType.probe
                                break
                            case 4:
                                node!.light?.type = SCNLight.LightType.spot
                                break
                            case 5:
                                node!.light?.type = SCNLight.LightType.area
                                break
                            default:
                                break
                        }
            ...
            case "setVolume":
                ...
                break
            case "setNodeText":
                if let name = arguments!["name"] as? String, let text = arguments!["text"] as? String {
                    let node = sceneView.scene.rootNode.childNode(withName: name, recursively: true)
                    let textNode = self.modelBuilder.textNodes[name]
            ...
            case "setTextColor":
                if let name = arguments!["name"] as? String,
                   let color = arguments!["color"] as? String {
                    let node = sceneView.scene.rootNode.childNode(withName: name, recursively: true)!
                    self.modelBuilder.setTextColor(node: node, color: color)
                }
                result(nil)
                break
            case "setBounds":
                if let name = arguments!["name"] as? String,
                   ...
                   let bgColor = arguments!["bgColor"] as? String {
                    let node = sceneView.scene.rootNode.childNode(withName: name, recursively: true)!
                    self.modelBuilder.setTextNodeBound(node: node, width: CGFloat(width), height: CGFloat(height), bgColor: bgColor)
                }
                result(nil)
                break
            case "setFont":
                if let name = arguments!["name"] as? String,
                   let fontSize = arguments!["fontSize"] as? Int,
                   let fontStyle = arguments!["fontStyle"] as? Int{
                    let node = sceneView.scene.rootNode.childNode(withName: name, recursively: true)
                    let textNode = self.modelBuilder.textNodes[name]
                ...
                break
            case "setTextAlign":
                if let name = arguments!["name"] as? String, let textAlign = arguments!["value"] as? Int {
                    let node = sceneView.scene.rootNode.childNode(withName: name, recursively: true)
                    let textNode = self.modelBuilder.textNodes[name]
                ...
                break
            case "setParent":
                if let parentName = arguments!["parent"] as? String, let childName = arguments!["child"] as? String {
                    let parent = sceneView.scene.rootNode.childNode(withName: parentName, recursively: true)
                    let child = sceneView.scene.rootNode.childNode(withName: childName, recursively: true)
...
//        }
    ...
    func addNode(dict_node: Dictionary<String, Any>, dict_anchor: Dictionary<String, Any>? = nil, parent: Any? = nil) -> Future<Bool, Never> {
        let type = dict_node["type"] as! Int
        return Future {promise in
            switch (type) {
                case 0, 4: // GLTF2 Model from Flutter asset folder
                    var targetPath = ""
                    
                    if (type == 4) {
                        let paths = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)
                        let documentsDirectory = paths[0]
                        let theFileName = (dict_node["uri"] as! NSString).lastPathComponent
                        targetPath = documentsDirectory.appendingPathComponent(theFileName).path
                    } else {
                        targetPath = FlutterDartProject.lookupKey(forAsset: dict_node["uri"] as! String)
                    }
                
                    // Add object to scene
                if let node: SCNNode = self.modelBuilder.makeNodeFromGltfGblFile(name: dict_node["name"] as! String,
                                                                                 modelPath: targetPath,
                                                                                 transformation: dict_node["transformation"] as? Array<NSNumber>,
                                                                                 light: dict_node["light"] as! Bool,
                                                                                 intensity: dict_node["intensity"] as! Int,
                                                                                 type:dict_node["iosLightType"] as! Int,
                                                                                 isEnabled: dict_node["isEnabled"] as! Bool) {
                        let children = dict_node["children"] as? Array<Dictionary<String, Any>>
                        if let anchorName = dict_anchor?["name"] as? String, let anchorType = dict_anchor?["type"] as? Int {
                            ...
                            if (parent != nil) {
                                let parentNode: SCNNode = parent as! SCNNode
                        ...
                        if (children != nil) {
                            for (_, item) in children!.enumerated() {
                                self.addNode(dict_node: item, parent: node)
                            }
                        }
                        
                        promise(.success(false))
                    } else {
                        self.sessionManagerChannel.invokeMethod("onError", arguments: ["Невозможно загрузить данную модель"])
                        promise(.success(false))
                    }
                    break
                case 1, 3: // GLB Model from the app's documents folder and flutter assets
                ...
                case 2: // web GLB Model
                    ...
                    .sink(receiveCompletion: {
                        ...
                        if let node: SCNNode = val {
                            ...
                            self.sessionManagerChannel.invokeMethod("onError", arguments: ["Unable to load gbl renderable \(dict_node["name"] as! String)"])
                            promise(.success(false))
                        }
                    }).store(in: &self.cancellableCollection)
                    break
                case 5: //create node from Text
                    if let node: SCNNode = self.modelBuilder.makeNodeFromText(name: dict_node["name"] as! String,
                                                                              text: dict_node["text"] as! String,
                                                                              transformation: dict_node["transformation"] as? Array<NSNumber>,
                                                                              color: dict_node["color"] as! String,
                                                                              bgColor: dict_node["backgroundColor"] as! String,
                                                                              width: dict_node["width"] as! Int,
                                                                              height: dict_node["height"] as! Int,
                                                                              fontSize: dict_node["fontSize"] as! Int,
                                                                              fontStyle: dict_node["fontStyle"] as! Int,
                                                                              textAlign: dict_node["textAlign"] as! Int,
                                                                              isEnabled: dict_node["isEnabled"] as! Bool) {
                            ...
                            self.sessionManagerChannel.invokeMethod("onError", arguments: ["Невозможно разместить текст"])
                            promise(.success(false))
                        }
                    break
                case 6, 9, 10, 11 : //media node 
                    self.sessionManagerChannel.invokeMethod("onError", arguments: ["Сейчас недоступен медиа-контент"])
                    break
                case 7, 8 : //image node
                    var targetPath = ""
                    ...
                    // Add object to scene
                if let node: SCNNode = self.modelBuilder.makeNodeFromImage(name: dict_node["name"] as! String,
                                                                           modelPath: targetPath,
                                                                           transformation: dict_node["transformation"] as? Array<NSNumber>,
                                                                           width: dict_node["width"] as! Int,
                                                                           height: dict_node["height"] as! Int,
                        ...
                        self.sessionManagerChannel.invokeMethod("onError", arguments: ["Невозможно загрузить данное изображение"])
                        promise(.success(false))
                    }
                    break
                default:
